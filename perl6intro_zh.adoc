...
.Perl 6 motto:
* TMTOWTDI (发音是 Tim Toady): 条条大路通罗马。
* 容易的事情应该保持容易，困难的事情应该变得容易，不可能的事情应该变得困难。

=== 行话
* *Perl 6*: 带有测试套件的语言规范。
通过该规范测试套件的实现被认为是 Perl 6。
* *Rakudo*: Perl 6 的编译器
* *Rakudobrew*: Rakudo 的安装管理器 
* *Panda*: Perl 6 的模块安装程序。
* *Rakudo Star*: 是一个包含 Rakudo, Panda, 和很多 Perl 6 模块与文档的包。
...
运行 Perl 6 代码可以通过使用 REPL（Read-Eval-Print 循环)来实现。实现方法是，打开命令行终端，输入 `perl6` 后回车。这会导致命令提示符 `>` 的出现。接着, 输入一行代码后回车。REPL 将会打印出该行的返回值。然后你可以输入另外的行, 或输入 `exit` 并回车以离开 REPL。

或者，你可以在文件中编写你的代码，保存后再运行。建议 Perl 6 脚本文件的扩展名设定为 `.pl6`。运行该文件的代码时只需将 `perl6 filename.pl6` 输入到命令行终端后回车即可。但不像 REPL, 这不会自动打印出每一行的结果：这里要求文件中的代码必须包含 `say` 那样的语句以打印输出。
...
[TIP]
--
Rakudo Star 捆绑了一个能帮你从 REPL 中获取大部分输出的行编辑器。

如果你安装了普通的 Rakudo 而不是 Rakudo Star，那么你的行编辑功能可能没有开启（利用上下方向键查询历史；左右方向键以编辑输入；TAB 键以完成当前行输入）功能。此时可以考虑运行以下命令，可以设置好上述功能：

* `panda install Linenoise` 能在 Windows, Linux 和 OSX 上工作

* `panda install Readline` 如果你在 Linux 上并且偏好 _Readline_ 库
--

=== 编辑器
因为大多数情况下我们会在文件中编写并存储 Perl 6 程序，因此我们需要一个优雅的而且能识别 Perl 6 语法的文本编辑器。

从个人角度来说，我推荐正在使用的 https://atom.io/[Atom]。这是一个时尚的文本编辑器，带有开箱即用的 Perl 6 语法高亮功能。https://atom.io/packages/language-perl6fe[Perl6-fe] 是相对于 Atom 的另一种选择，也可以高亮 Perl 6 的语法，该软件来源于原始的包，但存在很多 bug 需要修补和编辑。

社区里的其他同伴也有使用 http://www.vim.org/[Vim] ，https://www.gnu.org/software/emacs/[Emacs] 和 http://padre.perlide.org/[Padre] 的。

最新版本的 Vim 带有语法高亮的功能，Emacs 和 Padre 则需要另行安装额外的包。
...
=== 语法概要
Perl 6 是 *形式自由的*: 你可以使用(大多数时候)任何数量的空白.
...
* 它们可以包含破折号或撇好(除了第一个和最后一个字符), 前提是每个短划线或撇号的右侧有一个字母字符。
...
==== 引号
字符串要么被双引号分割要么被单引号分割Strings need to be delimited by either double quotes or single quotes.
...
| ! | 前缀(Prefix) | 将运算对象转换成布尔值并返回否运算结果 | !4 | False

| .. | 中缀(Infix) | Range 构造函数 |  0..5  | 创建0到5的整数列

| ..^ | 中缀(Infix) | Range 构造函数 |  0..^5  | 创建0到4的整数列

| ^.. | 中缀(Infix) | Range 构造函数 |  0^..5  | 创建1到5的整数列

| \^..^ | 中缀(Infix) | Range 构造函数 |  0\^..^5  | 创建1到4的整数列

| ^ | 前缀(Prefix) | Range 构造函数 |  ^5  | 和0..^5相同，创建0到4的整数列

| ... | 中缀(Infix) | 惰性列表构造函数 |  0...9999  |  只有接到请求时才返回元素

.2+| {vbar} .2+| 前缀(Prefix) .2+| Flattening 扁平化 | {vbar}(0..5)  | (0 1 2 3 4 5) 

<| {vbar}(0\^..^5)  <| (1 2 3 4)

|===

=== Reversed Operators 逆转运算符

在任何运算符前加`R`就可以得到调换运算符两边的参数得到的结果。

[cols=".^m,.^m,.^m,.^m", options="header"]
|===
| 正常运算符 | 结果 | Reversed Operator 逆运算符 | 结果
...
=== Reduction Operators 简约运算符

Reduction operators work on lists of values. 简约运算符应用于一列值。
They are formed by surrounding the operator with brackets `[]` 简约运算符由包裹有方括号`[]`运算符来表示。

[cols=".^m,.^m,.^m,.^m", options="header"]
|===
| 正常运算符 | 结果 | Reduction Operator 简约运算符 | 结果
...
NOTE: 可以前往https://docs.perl6.org/language/operators查看完整的运算符列表和它们的优先级。

== 变量
Perl 6 中的变量分为3类：标量、数组和哈希。
...
NOTE: 所有可应用于字符串的方法列表，请参照https://docs.perl6.org/type/Str
...
NOTE: 所有可应用于整数的方法列表，请参照https://docs.perl6.org/type/Int
...
NOTE: 所有可应用于有理数的方法列表，请参照https://docs.perl6.org/type/Rat
...
一般数据没有定义长度，所以能自动扩展长度。+
它可以接受任意个数的数字而不受限制。

对应的，我们可以创建一个定长数组。+
在这种数组中不能访问下标超过定义大小的元素。

可以通过在变量名后的方括号中制定最大元素个数来定义定长数组的长度: 
[source,perl6]
my @array[3];

这个数组最多可以储存3个变量，索引从 0 开始到 2 结束。
...
----
第一维的索引值3超出了范围（必须是0..2)
----

==== 多维数组
我们到现在为止看到的数组还都是一维的。 +
幸运的是，我们在Perl6中能定义多维数组。

[source,perl6]
my @tbl[3;2];

这是一个二维数组。
它的第一个维度最多可以有3个值，第二个维度最多可以有2个值。
...
.说明
`.push:(key => "value")` 追加一个新键值对。+
`.kv` 返回一个包含所有键值对的列表+
`.keys` 返回一个包含所有键的列表 +
`.values` 返回一个包含所有值的列表 +
我们可以通过哈希中特定值所对应的键来访问这个值。 `%hash<key>`

NOTE: 完整的哈希参考资料，请见https://docs.perl6.org/type/Hash
...
Perl 6 属于 *gradually typed* ；它同时支持*静态*和*动态*类型。
...
| Mu | The root of the Perl 6 type hierarchy Perl 6 的基础类型 | |

| Any | Default base class for new classes and for most built-in classes 新类的默认类型，同时也是大部分内部类的类型 | |
...
在Perl6中可以使用使用多种声明方式。其中 `my` 已经在上述例子中使用到。
...
=== Assignment vs. Binding 赋值 vs. 绑定
...
在 Perl 6 中我们能倒装待运行的代码和条件语句。 +
在待运行的代码和条件语句倒装后，条件语句总是优先被执行。
...
如果第一个 `with` 条件不能满足，替代的路径可以使用 `orwith` 来指定。+
`with` 和 `orwith` 可以同 `if` 与 `elsif` 做对比。
...
在 Perl 6 中的 `given`  等同与其他语言中的 switch。
但是要更加地强大。
...
如果在代码块中加了 `proceed`，那么就要在下一个成功匹配后才停止匹配。
...
当上面的代码运行是，终端会等你输入你的名字和回车键。
然后就会向你打招呼。

==== prompt
`prompt` 结合了 `print` 和 `get`。
...
在运行上述代码后，一个存储有新 score 的文件 _newdatafile.txt_ 会被创建。
...
在你要运行一下脚本的目录中，建立一个叫 `folder123` 的空文件夹和一个文件 `script123.pl6`
...
== Subroutines 子程序
=== Definition 定义
*子程序(Subroutines)* (也叫 *subs* 或 *functions*) 是一系列的功能性的打包(are a means of packaging a set of functionality.)。 +

定义子程序时以关键字 `sub` 起始。在定义之后你能通过子程序名(句柄？handle)来进行调用。+
让我们来看下面的例子：
...
上面的例子给我们展示了一个不需要输入参数的子程序。

=== Signature 函数签名
很多子程序需要多个输入才能运行。这些输入由 *参数* 提供。
一个子程序可以不定义或定义多个 *参数*。
子程序所定义的参数之个数与类型称为它的*函数签名*。
...
=== Multiple dispatch 多分配
我们可以定义多个有相同命名但有不同函数签名的的子程序。
当这样的子程序被调用的时候，运行环境会根据提供的参数之数量和类型判断那个版本的同名子程序来运行。
这种子程序和普通的子程序的定义方法是一样的，不过我们需要使用 `multi` 来代替原先的 `sub`。
...
=== Default and Optional Parameters 默认参数和可选参数
如果一个子程序被定义需要一个参数，但是我们调用它的时候没有提供所需的参数，那么这个子程序就不能运行。
...
=== 返回值
我们现在为止看到的子程序都在终端中打印一些文字。

一些时候我们需要让子程序 *返回* 值让我们能在我们的程序中能重复使用。

在一般的情景下，子程序代码的最后一行被默认为返回值。
[source,perl6]
.Implicit return
----
sub squared ($x) {
...
NOTE: 关于更多子程序和函数的资料，详见 https://docs.perl6.org/language/functions
...
注意：查看异常类型和相关方法的完整列表请到  [http://doc.perl6.org/type.html](http://doc.perl6.org/type.html) 
...
注意：更多关于 regexes 的东西, 查看 http://doc.perl6.org/language/regexes
...
注意：Perl 6 的模块目录可以在 https://modules.perl6.org/ 中找到。
...
=== 并发和异步
　
注意：关于并发和异步编程的更多信息, 请查看  http://doc.perl6.org/language/concurrency
